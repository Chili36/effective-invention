...
> **Source:** https://platform.claude.com/docs/en/about-claude/models/overview (official — verified May 25, 2026) · https://platform.claude.com/docs/en/about-claude/pricing · https://docs.anthropic.com/en/docs/about-claude/model-deprecations · https://claude.com/pricing#api
> **Scraped / verified:** 2026-05-25
...
> ✅ **May 25, 2026 re-verification:** All active model prices confirmed unchanged. **Max output corrections applied** — official API docs (`platform.claude.com/docs/en/about-claude/models/overview`) show higher max output values than previously recorded (Opus 4.7: 128k, Sonnet 4.6: 64k, Haiku 4.5: 64k). **Opus 4.6, Sonnet 4.5, Opus 4.5 moved to Legacy** — official Anthropic pricing page (`claude.com/pricing#api`) now lists these models under the "Legacy models" section as of May 25, 2026.
...
> **Updated May 25, 2026:** Claude Opus 4.7 remains Anthropic's most capable generally available model. Available in Bedrock console self-serve across 27 AWS regions.
...
| **Notable** | `xhigh` effort level (coding/agentic tasks); **task budgets beta** (caps token spend in agentic loops); `/ultrareview` (multi-agent code review in Claude Code); **3.75MP vision** (2,576px long edge); **new tokenizer** (up to 35% more tokens/request vs Opus 4.6 — same price/token but monitor per-request costs); SWE-bench Verified 87.6%; GPQA Diamond 94.2%; CursorBench 70%; US-only data residency: 1.1× multiplier |

> ✏️ **Max output correction (May 25, 2026):** A previous version of this card listed max output as 32,000 tokens (sync). Per the official Anthropic API docs (`platform.claude.com/docs/en/about-claude/models/overview`, verified May 25, 2026), the correct max output is **128,000 tokens** in sync mode.
...
> ✏️ **Fast Mode note:** Fast Mode ($30/$150) is available **only on Claude Opus 4.6** (now Legacy), not Opus 4.7. Per the official Anthropic pricing page (verified May 25, 2026).
...
| **Notable** | Improved agentic search; 1M context no premium; extended + adaptive thinking supported; recommended migration target from Sonnet 4 (retired Apr 20, 2026) and Sonnet 4.5 (now Legacy) |

> ✏️ **Max output correction (May 25, 2026):** A previous version of this card listed max output as 16,000 tokens (sync). Per the official Anthropic API docs (verified May 25, 2026), the correct max output is **64,000 tokens** in sync mode.
...
> ✏️ **Max output correction (May 25, 2026):** A previous version of this card listed max output as 8,000 tokens. Per the official Anthropic API docs (verified May 25, 2026), the correct max output is **64,000 tokens**.
...
|---|---|---|---|
| Claude Opus 4.7 | ❌ No | ✅ Yes | Best for agentic, coding tasks |
...
> Source: `platform.claude.com/docs/en/about-claude/models/overview`, verified May 25, 2026.
...
> **Update (May 25, 2026):** Claude Opus 4.6 has been moved to the **"Legacy models"** section on Anthropic's official pricing page (`claude.com/pricing#api`, verified May 25, 2026). The model remains API-accessible at the same price. This is the **only Claude model with Fast Mode** ($30/$150 — 6× base pricing for low latency). Migrate to **Claude Opus 4.7** for the current flagship.
...
| **Input price** | $5.00 / MTok |
| **Output price** | $25.00 / MTok |
| **Fast Mode (input)** | $30.00 / MTok *(only Opus model with Fast Mode — NOT available on Opus 4.7)* |
...
| **Availability** | API, AWS Bedrock, Google Vertex AI, Microsoft Foundry |
| **Migration** | Upgrade to **Claude Opus 4.7** ($5/$25) — same price, improved performance, 128k max output |

---

### ⚠️ LEGACY — Claude Sonnet 4.5 *(Moved to Legacy May 2026)*

> **Update (May 25, 2026):** Claude Sonnet 4.5 has been moved to the **"Legacy models"** section on Anthropic's official pricing page (`claude.com/pricing#api`, verified May 25, 2026). The model remains API-accessible. The 1M-token context window beta (`context-1m-2025-08-07`) was **retired April 30, 2026**. Migrate to **Claude Sonnet 4.6** ($3/$15, 1M context at standard pricing, 64k max output).
...
> **Update (May 25, 2026):** Claude Opus 4.5 has been moved to the **"Legacy models"** section on Anthropic's official pricing page (`claude.com/pricing#api`, verified May 25, 2026). The model remains API-accessible.
...
| **Context window** | 200,000 tokens |
| **Availability** | API, AWS Bedrock, Google Vertex AI, Microsoft Foundry |
| **Migration** | Upgrade to **Claude Opus 4.7** ($5/$25) — same price, 1M context, 128k max output, much better performance |
...
> **Status:** Per the official Anthropic model deprecations page (docs.anthropic.com/en/docs/about-claude/model-deprecations, verified May 25, 2026), **Claude Sonnet 4 was retired on April 20, 2026**. All API requests now return errors.
...
> **Status:** Per the official Anthropic model deprecations page (docs.anthropic.com/en/docs/about-claude/model-deprecations, verified May 25, 2026), **Claude Opus 4 was retired on April 20, 2026**. All API requests now return errors.
...
> **Status:** Per the official Anthropic model deprecations page (verified May 25, 2026), Claude Haiku 3.5 was **retired on the Claude API on February 19, 2026**. API calls will fail. Bedrock/Vertex AI may retain access on their own schedules.
...
> **Status:** Claude Haiku 3 was officially retired February 19, 2026. All API requests to `claude-3-haiku-20240307` now return errors.
...
> **Status:** Per the official Anthropic model deprecations page, Claude Sonnet 3.7 was **retired on October 28, 2025**. The model is no longer operational.
...
| **Fast Mode (Opus 4.6 Legacy only)** | $30/$150 input/output (6× standard) — only available on Opus 4.6 (now Legacy); NOT on Opus 4.7 |
| **Thinking modes** | Adaptive thinking: Opus 4.7 + Sonnet 4.6. Extended thinking: Sonnet 4.6 + Haiku 4.5 (not on Opus 4.7). |

---

*Sources last verified: May 25, 2026 against the official Anthropic models overview page at `platform.claude.com/docs/en/about-claude/models/overview` (max output corrections + thinking capabilities applied), official pricing page at `claude.com/pricing#api` (Opus 4.6 / Sonnet 4.5 / Opus 4.5 moved to Legacy section confirmed), and model deprecations at `docs.anthropic.com/en/docs/about-claude/model-deprecations` (retirement dates confirmed). No price changes detected since May 21, 2026.*
