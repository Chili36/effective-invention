...
> **Last updated:** 2026-08-10 (refresh #30)
...
> 🔓 **Claude Fable 5 + Mythos 5 RESTORED** — the US export control directive from June 12, 2026 was **lifted June 30, 2026**. Fable 5 is available globally starting July 1, 2026. Mythos 5 restored for approved Project Glasswing orgs since June 26, 2026.
...
> ✅ **Re-verified August 10, 2026** directly against the live `platform.claude.com/docs/en/about-claude/pricing` page and `anthropic.com/news` (through Aug 7) — every price point above confirmed unchanged; no new model releases found since Opus 5. Only non-pricing governance/safety news items appeared this cycle (Fable 5 biology-safeguard update, new Chief Global Affairs Officer, cybersecurity-evaluation research post).
...
| 📉 **GPT-5.6 Luna** *(Fastest/cheapest tier — 🆕 new default for ChatGPT Free/Go)* | $0.20 / $0.40* | $0.02 / $0.04* | $1.20 / $1.80* | **~1.05M tokens** | ChatGPT, Codex, API — self-serve |
...
> 🆕 **August 6, 2026 — ChatGPT consumer product update (no API pricing impact):** GPT-5.6 Luna became the **default model for ChatGPT Free and Go users**, with **unlimited text chats** (previously rate-limited) and a new "Think" button for harder questions. GPT-5.6 Sol in the ChatGPT Chat surface (Plus/Pro) was separately updated for more focused, more factually reliable answers, plus a new effort slider. **API token pricing for `gpt-5.6-luna` and `gpt-5.6-sol` is unchanged** — this is a ChatGPT consumer availability/quality update only.
...
> 🆕 **GPT-Live-1 / GPT-Live-1 mini** — OpenAI's full-duplex voice models, launched July 8, 2026 into ChatGPT Voice. Bundled into ChatGPT plans only (no separate charge); **no API pricing published yet** ("coming soon"), still unpriced as of Aug 10. This is distinct from the Realtime API (`gpt-realtime-2.1`/`2.1-mini`), which remains the only developer-facing voice API today.
...
> ✅ Re-verified August 10, 2026 against the live `developers.openai.com/api/docs/pricing` and `developers.openai.com/api/docs/models` pages — **no API price changes this cycle**; every price point for GPT-5.6 Sol/Terra/Luna, GPT-5.5, GPT-5.5 Pro, and the GPT-5.4 family confirmed unchanged from Aug 3. Only a ChatGPT consumer product update (Luna default for Free/Go) was newly discovered.
...
> 🆕 **New discovery (Aug 4, 2026) — Shieldstral 1.0:** a 3B/3.8B open-weight (Apache 2.0), policy-adaptive multimodal safety classifier for text and images. **No hosted API pricing published** — self-host only (single 16GB GPU). Not the same product as the already-priced `mistral-moderation-2603` endpoint. Not added as a priced row per this tracker's verification-before-tracking policy; see `mistral.md` Discovery-Only section.
...
> ✅ Re-verified August 10, 2026 against the live `mistral.ai/pricing/api` page — no price changes or new priced releases detected since Aug 3; one new unpriced open-weight discovery (Shieldstral 1.0).
...
|---|---|---|---|
| Anthropic | 1 | [anthropic.md](./anthropic.md) | Full model cards — Claude Opus 5 ($5/$25, replaces Opus 4.8 as default) · 🔓 Fable 5 + Mythos 5 restored · Claude Sonnet 5 (new default) · ✅ Opus 4.7 Fast Mode removal confirmed · 🔧 Tools & Agents pricing · re-verified Aug 10, no changes |
| OpenAI | 1 | [openai.md](./openai.md) | Full model cards — no API price changes since Aug 3 · 🆕 Aug 6 ChatGPT product update (Luna default for Free/Go, Sol chat tuning — non-pricing) · GPT-5.6 GA since July 9, current flagship family · 🆕 GPT-Live-1/mini voice (ChatGPT-only, unpriced API) · re-verified Aug 10 |
...
| Mistral AI | 1 | [mistral.md](./mistral.md) | Full model cards — ✅ re-verified Aug 10, no pricing changes · 🆕 Shieldstral 1.0 (open-weight safety classifier, unpriced) flagged as discovery-only · 🔭 Robostral Navigate (robotics, unpriced) also discovery-only · Leanstral 1.5 retirement Sep 30 · OCR 4 (Jun 23) · Small 4 price +50%/+100% |
...
|---|---|---|---|
| 2026-08-10 | OpenAI | **All active models, Tools, Realtime, Image, Video, Transcription, Deep Research, Computer Use, Codex** | ✅ RE-VERIFIED — Independently re-checked the live `developers.openai.com/api/docs/pricing` page in full. **Zero API price changes detected** since the Aug 3, 2026 refresh. |
| 2026-08-10 | OpenAI | **ChatGPT consumer product update (non-pricing)** | 🆕 **DISCOVERED** — Aug 6, 2026: GPT-5.6 Luna became the default model for ChatGPT Free/Go users with unlimited text chats and a new "Think" button; GPT-5.6 Sol was separately tuned for more focused/factual answers in ChatGPT Chat (Plus/Pro), with a new effort slider. **No API token pricing changed** — consumer availability/quality update only. |
| 2026-08-10 | Anthropic | **All active + legacy models** | ✅ RE-VERIFIED — Independently re-fetched the live `platform.claude.com/docs/en/about-claude/pricing` page in full (model, batch, fast-mode, and tool-use tables). **Zero price changes detected.** Checked `anthropic.com/news` through Aug 7, 2026 — only non-pricing governance/safety items found (Fable 5 biology-safeguard update, new Chief Global Affairs Officer hire, cybersecurity-evaluation research post, open-weights policy statement). |
| 2026-08-10 | Mistral | **Shieldstral 1.0** | 🆕 **NEW DISCOVERY (not priced, not tracked as a full model)** — Mistral published "Introducing Shieldstral" (Aug 4, 2026), a 3B/3.8B open-weight (Apache 2.0), policy-adaptive multimodal safety classifier for text and images. No hosted API pricing published — self-host only (single 16GB GPU). Distinct from the already-priced `mistral-moderation-2603` endpoint. Flagged for awareness in `mistral.md`'s Discovery-Only section per this tracker's verification-before-tracking policy. |
| 2026-08-10 | Mistral | **All active models + legacy table** | ✅ RE-VERIFIED — Independently checked every price point against `mistral.ai/pricing/api`; no price changes or new priced releases detected since the Aug 3, 2026 refresh. |
...
- 🆕 **Unpriced/discovery-only items** (e.g., OpenAI's GPT-Live voice models, Mistral's Shieldstral 1.0, Robostral Navigate, and upcoming open-weight model family) are noted for awareness but are **not** given a full tracked model card until the provider publishes official pricing/specs, per this tracker's verification-before-tracking policy.
...
- 🆕 **August 4, 2026:** Mistral published Shieldstral 1.0, an open-weight (Apache 2.0) policy-adaptive multimodal safety classifier with no published hosted API pricing — discovery-only, self-host only.
- 🆕 **August 6, 2026:** OpenAI made GPT-5.6 Luna the default ChatGPT model for Free/Go users with unlimited text chats, and refreshed GPT-5.6 Sol's tone/accuracy in ChatGPT Chat for Plus/Pro — a consumer product update with **no API pricing impact**.
...
- 📝 **August 10, 2026:** This refresh independently re-verified Anthropic, OpenAI, and Mistral pricing against each provider's live pricing/docs pages and news feeds. **No API token pricing changes were found on any of the three Tier 1 providers this cycle** — every price point for Anthropic (Fable 5, Mythos 5, Opus 5, Opus 4.8, Sonnet 5, Sonnet 4.6, Haiku 4.5, and all legacy models), OpenAI (GPT-5.6 Sol/Terra/Luna, GPT-5.5, GPT-5.5 Pro, GPT-5.4 family, GPT-4.1 family, o-series, Realtime, Image, Video, Transcription, Deep Research, Computer Use, Codex, Tools), and Mistral (Medium 3.5, Large 3, Small 4, Magistral, Codestral, Devstral, Leanstral 1.5, Voxtral family, OCR 4, Ministral 3, Classifier APIs, embeddings, Agent API tools) was independently re-checked and matches the prior refresh exactly. **Two discovery-only, non-pricing items were newly found:** (1) OpenAI's August 6, 2026 ChatGPT product update made GPT-5.6 Luna the default model for Free/Go users with unlimited text chats (no API price change); (2) Mistral's August 4, 2026 release of **Shieldstral 1.0**, an open-weight (Apache 2.0) policy-adaptive multimodal safety classifier with no published hosted API pricing, was added to `mistral.md`'s Discovery-Only section alongside Robostral Navigate. Google Gemini and OpenRouter Picks tables reflect the last confirmed figures from a prior refresh and were not re-verified this cycle.
